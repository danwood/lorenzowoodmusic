  ...
  <body><?php if (!$album) { ?>
    ...
    <div id="redeem_console">
      ...
      <div id="cover-art-container">
        <div id="cover-art"><img src="/album_art/<?php echo htmlspecialchars($album['imageName']); ?>" alt="<?php echo htmlspecialchars($album['title']); ?>"></div>
      </div>
      <form id="downloader">
        <input type="hidden" id="a_input" name="a" value="<?php echo $album['ID']; ?>">
        <input type="hidden" id="c_input" name="c" value="<?php echo $code; ?>"><?php
        if ($iOSDevice) {
        ?>
        ...
        <div id="list-container">
          ...
          <div class="song">
            <div class="song-meta-data">&nbsp;</div><span class="song-duration">&nbsp;</span><?php
            	if (!$iOSDevice) {	// don't output download, but just in case, it will be hidden by JS as well.
            ?><span class="song-download">
              <button class="download-button" type="button" name="t" value="0">Download all</button></span>
          </div>
          <div class="song">Individual Tracks:</div><?php
          }
          $trackIndex = 0;
          ...
          <div class="song">
            ...
            <div class="song-download">
              <button class="download-button" type="button" name="t" value="<?php echo $track['ID']; ?>">Download</button>
            </div><?php
            	}
            ?>
          </div><?php
    ...
    <script>
      $('button.download-button').click(function (evt) {
      	evt.preventDefault();
