...
?><!DOCTYPE html>
<html lang="en-us">
  <head><link rel="stylesheet" href="redeem.css" /></head>
  <body><?php if (!$album) { ?>
    <p>
    ...
    <div id="redeem_console">
      ...
      <div id="cover-art-container">
        <div id="cover-art"><img amplitude-song-info="cover_art_url" amplitude-main-song-info="true"></div>
      </div>
      <form id="downloader">
        <input type="hidden" id="a_input" name="a" value="<?php echo $album['ID']; ?>">
        <input type="hidden" id="c_input" name="c" value="<?php echo $code; ?>">
        <div id="time-container"><span class="current-time"><span class="amplitude-current-minutes" amplitude-main-current-minutes="true"></span>:<span class="amplitude-current-seconds" amplitude-main-current-seconds="true"></span></span>
          <input class="amplitude-song-slider" type="range" amplitude-main-song-slider="true" step=".1"><span class="duration"><span class="amplitude-duration-minutes" amplitude-main-duration-minutes="true"></span>:<span class="amplitude-duration-seconds" amplitude-main-duration-seconds="true"></span></span>
        </div>
        <div id="central-controls">
          <div id="central-controls-grouped">
            <div class="amplitude-prev" id="previous"></div>
            <div class="amplitude-play-pause" id="play-pause" amplitude-main-play-pause="true"></div>
        ...
        <div id="list-container"><span id="format-container">
          ...
          <div class="song amplitude-song-container amplitude-play-pause" amplitude-song-index="<?php echo $trackIndex++; ?>">
            ...
            <div class="song-meta-data"><span class="track-number"><?php echo htmlspecialchars($track['trackNumber']); ?>.</span><span class="song-title"><?php echo htmlspecialchars($track['title']); ?></span></div><span class="song-duration"><?php echo htmlspecialchars($track['duration']); ?></span><?php
            	if (!$iOSDevice) {
            ?>
            <div class="song-download">
              <button class="download" type="button" name="t" value="<?php echo $track['ID']; ?>">Download</button>
            </div><?php
            	}
            ?>
          </div><?php
